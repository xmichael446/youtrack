...
import { Coins, Clock, Calendar, Zap, BookOpen, CheckCircle2, ShoppingBag } from 'lucide-react';
                        ...
                        <table className="min-w-full divide-y divide-gray-50 dark:divide-slate-800">
                            <thead className="bg-gray-50/50 dark:bg-slate-800/50">
                                <tr>
                                    ...
                                    <th scope="col" className="px-6 py-4 text-right text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-[0.2em]">{t('xp')}</th>
                                    <th scope="col" className="px-6 py-4 text-right text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-[0.2em]">{t('coins')}</th>
                                </tr>
                            </thead>
                                ...
                                {displayActions.map((item) => (
                                    <tr key={item.id} className="hover:bg-brand-primary/5 dark:hover:bg-brand-primary/5 transition-all group/row">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                <div className="flex-shrink-0 h-9 w-9 rounded-lg bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 flex items-center justify-center mr-3 shadow-sm group-hover/row:scale-110 group-hover/row:bg-white dark:group-hover/row:bg-slate-700 transition-all">
                                                    {getIcon(item.action)}
                                                </div>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right">
                                            {item.xpChange !== null ? (
                                                <span className="inline-flex items-center px-3 py-1.5 rounded-lg text-[11px] font-extrabold bg-brand-primary/10 text-brand-primary border border-brand-primary/10 tabular-nums">
                        ...
                        <div key={item.id} className="p-4 active:bg-gray-50 dark:active:bg-slate-800/50 transition-colors hover:bg-gray-50/50 dark:hover:bg-slate-800/30">
                            <div className="flex items-start justify-between mb-3">
                                <div className="flex items-center min-w-0">
                                    <div className="flex-shrink-0 h-10 w-10 rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 flex items-center justify-center mr-3 shadow-sm">
                                        {getIcon(item.action)}
                                    </div>
                            ...
                            <div className="flex items-center space-x-3 pl-[3.25rem]">
                                {item.xpChange !== null && (
                                    <span className="inline-flex items-center px-2 py-1 rounded-md text-[10px] font-bold bg-brand-primary/5 text-brand-primary border border-brand-primary/10 tabular-nums">
